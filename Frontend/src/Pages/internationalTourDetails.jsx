...
const TourDetails = () => {
  ...
  const { data: tour, loading: tourLoading, error: tourError } = useFetch(`${BASE_URL}/internationalTour/${id}`);

  // Function to fetch tour internity data
  const fetchInternityData = async () => {
    try {
      if (showInternity) {
        // If the internity data is already shown, hide it on the second click
        setShowInternity(false);
        setSelectedDay(null); // Reset selected day when hiding internity data
      } else {
        const response = await fetch(`${BASE_URL}/Internity/${id}`);
        const data = await response.json();
        setInternityData(data);
        setShowInternity(true); // Set showInternity to true when data is fetched successfully
      }
    } catch (error) {
      setShowInternity(false); // Set showInternity to false if there's an error
    }
  };
  ...
  const fetchDatesAndCostingData = async () => {
    try {
      if (showDatesAndCosting) {
        // If the Dates and Costing data is already shown, hide it on the second click
        setShowDatesAndCosting(false);
      } else {
        const response = await fetch(`${BASE_URL}/Internity/it/${id}`); // Adjust the API endpoint URL as needed
        const data = await response.json();
        setDatesAndCostingData(data);
        setShowDatesAndCosting(true); // Set showDatesAndCosting to true when data is fetched successfully
      }
    } catch (error) {
      setShowDatesAndCosting(false); // Set showDatesAndCosting to false if there's an error
    }
  };

  // Function to handle "Inclusion" button click
                        ...
                        {internityData.miniData.map((data, index) => (
                          <div key={index}>
                            <div className='sc-Outer'>
                              ...
                              <span><i className={`ri-menu-${selectedDay === index ? 'fold-fill' : 'add-fill'}`} onClick={() => handleDayClick(index)}></i></span>
                            </div>
                            {selectedDay === index && (
                              <>
                                {/* Info for each day */}
